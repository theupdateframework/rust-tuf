...
fn parser<'a, 'b>() -> App<'a, 'b> {
    App::new("tuf")
        ...
        .about("CLI tool managing TUF repositories")
        ...
        .subcommand(SubCommand::with_name("keygen")
            ...
            .arg(Arg::with_name("type")
                ...
                .help("The type of key to generate. Note: rsa-XXX requires `openssl` to be on the PATH.")
            )
        )
}
    ...
    let der = match typ {
        "ed25519" => {
            Ed25519KeyPair::generate_pkcs8(&SystemRandom::new())
                .map_err(|e| format!("Failed to generate key: {:?}", e))?.to_vec()
        },
...
fn rsa_gen(size: u32) -> Result<Vec<u8>, String> {
    let gen = Command::new("openssl")
        .args(&["genpkey", "-algorithm", "RSA", "-pkeyopt", &format!("rsa_keygen_bits:{}", size),
                "-pkeyopt", "rsa_keygen_pubexp:65537", "-outform", "der"])
        .stdout(Stdio::piped())
        .spawn()
        .map_err(|e| format!("{:?}", e))?;
    ...
    match pk8.stdin {
        Some(ref mut stdin) => {
            for byte in gen.stdout.ok_or("Failed to get stdout handle")?.bytes() {
                stdin.write(&[byte.map_err(|e| format!("Couldn't read byte: {:?}", e))?])
